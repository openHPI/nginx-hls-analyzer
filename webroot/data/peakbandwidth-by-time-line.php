<?php
// $Id: peakbandwidth-by-time-line.php 10 2009-09-21 13:01:03Z bvamos $
...
MAX(ROUND(`sc-stream-bytes`/1024*8/`x-duration`, 0)) peakbw
FROM fmslog
...
while ($row = mysql_fetch_assoc($result)) {
    $data[$row['t']] = (int) $row['peakbw'];
    if((int) $row['peakbw']>$ymax) $ymax = (int) $row['peakbw'];
}
...
$charttitle = new title( "Peak Bandwidth - $title ($from_date - $to_date)" );
